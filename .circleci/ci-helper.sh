...
function runDanger {
    if [[ $CIRCLE_BRANCH == *"pull"* ]]; then
        ...
        if [ -z "${CURRENT_LIB}" ]; then
            DANGER_GITHUB_API_TOKEN="${DANGER_GITHUB_API_TOKEN}" danger --dangerfile=.circleci/Dangerfile_PR.rb --danger_id=PR-Check --verbose
        else
            ...
            DANGER_GITHUB_API_TOKEN="${DANGER_GITHUB_API_TOKEN}" danger --dangerfile=.circleci/Dangerfile_Lib.rb --danger_id="${CURRENT_LIB}" --verbose
        fi
    else
